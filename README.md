...
- **Day streaks** — completing a work session on consecutive calendar days
  builds a streak, shown in the tooltip and settings popup (🔥 resets if a
  day is missed).
- **Milestones** — points roll up into milestones (100 pts each by default,
  configurable via the `milestoneStep` widget setting). Progress toward the
  next one shows in the tooltip and settings popup.
- **Session history** — a 󰋚 button in settings opens a log of your last 8
  completed sessions with duration, response time, and points earned.
- Optional local-AI messages (off by default): open the 󰒓 "Advanced
  settings" popup (top-right of the settings popup) and turn on "AI-generated
  alert text" to replace the hardcoded line with one freshly generated by a
  local `pi` CLI + Ollama model, if you have one configured on your machine.
  Nothing is called out to unless you turn this on.
